...
class W {
  ...
  update(deltaInfo: { delta: number }) {
    ...
    const delta = Math.min(deltaInfo.delta, 0.033);

    let startIdx = 0;

    // Handle cursor-following ball - always active for smooth interaction
    startIdx = 1;
    const firstVec = new Vector3().fromArray(positionData, 0);
    firstVec.lerp(center, 0.2).toArray(positionData, 0);
    ...
    for (let idx = startIdx; idx < config.count; idx++) {
      ...
      // Clamp velocity
      if (vel.length() > config.maxVelocity) {
        vel.normalize().multiplyScalar(config.maxVelocity);
      }
    ...
    // Handle collisions with optimized algorithm
    for (let idx = startIdx; idx < config.count; idx++) {
      const base = 3 * idx;
      const pos = new Vector3().fromArray(positionData, base);
      const vel = new Vector3().fromArray(velocityData, base);
      ...
      for (let jdx = idx + 1; jdx < config.count; jdx++) {
        ...
        if (dist < sumRadius && dist > 0.0001) {
          ...
          const direction = diff.divideScalar(dist);
          
          // Separate balls based on mass (size)
          const totalMass = radius + otherRadius;
          const ratio1 = otherRadius / totalMass;
          const ratio2 = radius / totalMass;
          
          const correction = direction.multiplyScalar(overlap * 0.5);
          
          pos.sub(correction.clone().multiplyScalar(ratio1));
          otherPos.add(correction.clone().multiplyScalar(ratio2));
          
          // Exchange velocities along collision normal
          const relVel = new Vector3().subVectors(otherVel, vel);
          const speed = relVel.dot(direction);
          
          if (speed < 0) {
            const impulse = direction.multiplyScalar(speed * 0.5);
            vel.add(impulse.clone().multiplyScalar(ratio1));
            otherVel.sub(impulse.clone().multiplyScalar(ratio2));
          }
      ...
      // Cursor ball collision
      if (config.controlSphere0) {
        const centerPos = new Vector3().fromArray(positionData, 0);
        const diff = new Vector3().subVectors(centerPos, pos);
        const dist = diff.length();
        ...
        if (dist < sumRadius && dist > 0.0001) {
          ...
          pos.sub(correction);
          vel.sub(correction.clone().multiplyScalar(0.2));
